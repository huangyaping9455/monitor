...
<template>
  <div class="main">
    ...
    >
      ...
      <el-table-column label="是否入网" align="center" width="100">
        <template slot-scope="{ row }">
          <span>{{ row.joinState == 1 ? "是" : "否" }}</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="olGpstime"
      ...
      <el-table-column label="是否在线" align="center" width="100">
        <template slot-scope="{ row }">
          <span>{{ row.olState == 1 ? "是" : "否" }}</span>
        </template>
      </el-table-column>
      <el-table-column
          ...
          <el-button
            ...
            @click="
              $router.push({
                path: '/completeTrack',
                query: {
                  cheliangpaizhao: row.cheliangpaizhao,
                  date: row.dateShow,
                },
              })
            "
          >
            {{ row.continuationRateShow }}
          </el-button>
        </template>
      </el-table-column>
      <el-table-column
        prop="unDriftingPoiCount"
        label="正常轨迹数"
        align="center"
        width="100"
        :show-overflow-tooltip="true"
...
export default {
  ...
  methods: {
    ...
    // 请求数据判断
    getDate(page) {
      this.getVehDayTJ(page);
    },
    //地区报警处理率
    async getVehDayTJ(current = 1) {
      current = Number(current);
      this.loading = true;
      let [err, data] = await dataAnalysisApi.awaitWrap(
        dataAnalysisApi.getVehDayTJ({
