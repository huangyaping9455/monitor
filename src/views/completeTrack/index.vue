...
export default {
  ...
  created() {
    if (this.$route.query.cheliangpaizhao) {
      this.form.begintime = this.$route.query.date;
      this.form.cheliangpaizhao = this.$route.query.cheliangpaizhao;
    }
    this.getVehContinuationDetailTJ();
  },
  computed: {
    ...mapGetters({
  ...
  methods: {
    ...
    // 请求数据判断
    getDate(page) {
      this.getVehContinuationDetailTJ(page);
    },
    //地区报警处理率
    async getVehContinuationDetailTJ(current = 1) {
      current = Number(current);
      this.loading = true;
      let [err, data] = await dataAnalysisApi.awaitWrap(
        dataAnalysisApi.getVehContinuationDetailTJ({
