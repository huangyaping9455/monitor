...
<template>
  <div class="dataAnalysis">
    ...
    <div class="home-footer">
      ...
      >
        <div class="title">
          <span>运营商单车报警比排名</span>
          <span class="more" @click="linkto('/YYSEnterprise')">更多>></span>
        </div>
        <div class="tablebox">
          <div class="tr">
            <span class="tdl">排名</span>
          ...
          <vueSeamlessScroll
            :data="enterpriseYYSList"
            :class-option="classOption"
            class="wrap"
          >
            <div
              class="tr"
              v-for="(item, index) in enterpriseYYSList"
              :key="index"
            >
              <p class="tdl">
                <span>{{ index + 1 }}</span>
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      //运营商报警排名
      this.getYYSZFQYBJPMList(this.userinfo.deptId);
    },
    // 获取报警处理情况(月)
    ...
    // 运营商报警排名
    async getYYSZFQYBJPMList(deptId) {
      let [err, data] = await dataAnalysisApi.awaitWrap(
        dataAnalysisApi.getYYSZFQYBJPMList({
          deptId: deptId,
        })
      );
      this.loading5 = false;
      if (data) {
        this.enterpriseYYSList = data;
        // }
      } else {
        this.$message.error(err);
      }
    ...
    echartdblclick(el, isxiazhuan) {
      ...
      this.getYYSZFQYBJPMList(el.data.zhengfuid);
