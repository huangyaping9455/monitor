...
export default {
  ...
  methods: {
    ...
    // 请求数据判断
    getDate(page) {
      this.getZFVehicleWeiHuPage(page);
    },
    //地区报警处理率
    async getZFVehicleWeiHuPage(current = 1) {
      current = Number(current);
      this.loading = true;
      let [err, data] = await dataAnalysisApi.awaitWrap(
        dataAnalysisApi.getZFVehicleWeiHuPage({
